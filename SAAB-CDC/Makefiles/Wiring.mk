...
#
#
# Last update: Dec 09, 2015 release 4.0.7

# WIRING SUPPORT IS PUT ON HOLD
#WARNING_MESSAGE = Support for Wiring is put on hold.
...
#
ifeq ($(USER_LIBRARY_DIR)/Wiring/preferences.txt,)
    $(error Error: run Wiring once and define the sketchbook path)
endif

ifeq ($(wildcard $(SKETCHBOOK_DIR)),)
    SKETCHBOOK_DIR = $(shell grep sketchbook.path $(USER_LIBRARY_DIR)/Wiring/preferences.txt | cut -d = -f 2)
endif
ifeq ($(wildcard $(SKETCHBOOK_DIR)),)
...
VARIANT_OBJS    = $(patsubst $(APPLICATION_PATH)/%.cpp,$(OBJDIR)/%.cpp.o,$(VARIANT_CPP_SRC)) # */
...
ifneq ($(APP_LIBS_LIST),0)
    ...
    APP_LIB_H_SRC   = $(wildcard $(patsubst %,%/*.h,$(APP_LIBS))) # */
    ...
    BUILD_APP_LIB_H_SRC   = $(wildcard $(patsubst %,%/*.h,$(BUILD_APP_LIBS))) # */
